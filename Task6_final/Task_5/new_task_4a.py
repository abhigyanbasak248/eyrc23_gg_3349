...
import json
model = YOLO('/home/shagnik/Downloads/best(1).pt')
...
labels_to_print = ["blank", "Combat", "Destroyed buildings", "Fire", "Humanitarian Aid and rehabilitation", "Military Vehicles"]
mid_y=[]
ca=[]
i=1
while True:
    ...
    if len(mid_y)>0 and i>0:
        ...
        for j in range(0,5):
            ...
            n1.append(labels_to_print[s_c[j]])
        # print(n1)
        ele_dict=dict(zip(['E','D','C','B','A'],n))
        ele_dict1=dict(zip(['E','D','C','B','A'],n1))
        # print(ele_dict1)
        new_dict = {}
        for key, value in zip(ele_dict1.keys(), ele_dict1.values()):
            if (value != "blank"):
                new_dict[key] = value
        print(new_dict)
        if(len(s_c))==5:
